...
function Deliverables({ items }) {
  ...
  return (
    ...
    >
      ...
      {/* Numbered spec list rather than a fixed-column grid: deliverable
          counts vary per service, and a rigid grid left a dead cell in the
          last row (5 items in 3 columns → empty bottom-right box). A divided
          list fills exactly its content, whatever the count. */}
      <ol
        ref={gridRef}
        className="flex flex-col divide-y divide-muted/40 border-y border-muted/40"
      >
        {items.map((it, i) => (
          <li
            key={i}
            data-stagger-item
            data-col={i}
            className="group flex items-center gap-6 py-7 md:gap-10 md:py-8"
          >
            <span
              data-reveal="icon"
            ...
            <span
              data-reveal="icon"
              aria-hidden
              className="h-px w-10 shrink-0 bg-muted/50 transition-all duration-500 group-hover:w-16 group-hover:bg-accent"
            />
            <MaskedLine className="text-lg font-medium leading-snug text-foreground/85 md:text-xl">
              {it}
            </MaskedLine>
          </li>
        ))}
